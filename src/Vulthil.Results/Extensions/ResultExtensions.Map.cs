...
public static partial class ResultExtensions
{
    ...
    public static async Task<Result<T2>> MapAsync<T2>(this Result result, Func<Task<T2>> map) =>
        result.IsSuccess ? Result.Success(await map().ConfigureAwait(false)) : Result.Failure<T2>(result.Error);
    ...
    public static async Task<Result<T2>> MapAsync<T1, T2>(this Result<T1> result, Func<T1, Task<T2>> map) =>
        result.IsSuccess ? Result.Success(await map(result.Value).ConfigureAwait(false)) : Result.Failure<T2>(result.Error);
