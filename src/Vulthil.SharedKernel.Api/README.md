# Vulthil.SharedKernel.Api

[![NuGet](https://img.shields.io/nuget/v/Vulthil.SharedKernel.Api)](https://www.nuget.org/packages/Vulthil.SharedKernel.Api)

API-layer helpers for endpoints, controllers, and cross-cutting HTTP concerns.

## Install

`dotnet add package Vulthil.SharedKernel.Api`

## Docs

Usage patterns: https://github.com/Vulthil/Vulthil.SharedKernel/tree/main/docs/articles/packages
